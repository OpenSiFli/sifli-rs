...
impl Ptab {
    pub fn new(contents: &String) -> Result<Self> {
        ...
        let mut set_ftab_value =
            |ftab_addr_str: &str, ftab_name: &str, addr: u32, size: u32| -> Result<()> {
        ...
        let (_flash_table_start_addr, flash_table_size) =
            find_by_tag(&partition_table, "FLASH_TABLE")?.unwrap();
...
mod tests {
    ...
    fn test_parse_ptab() {
        ...
        assert_eq!(
            partition_table.partition_table.len(),
            4,
            "Expected 4 memory segments"
        );
        ...
        assert!(
            first_region
                ...
                .as_ref()
                .unwrap()
                .contains(&"FLASH_TABLE".to_string())
        );
    }
}
