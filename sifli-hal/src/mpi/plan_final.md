...
### 1.1 Progress Snapshot (as of February 28, 2026)
...
1. Run full hardware matrix for all updated MPI examples (`blocking`, `auto-detect`, `async`) on `sf32lb52x`.
2. Capture logs for async off-XIP execution path and same-instance XIP rejection path.
3. Optionally add runner script for sequential hardware execution + log capture.
