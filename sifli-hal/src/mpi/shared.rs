...
pub(super) fn ram_read_jedec_id(regs: Regs, cmd: u8) -> Result<u32, ()> {
    ...
    if !ram_wait_not_busy(regs) {
        return Err(());
    }

    // 3 bytes (dlen = len - 1)
    ram_read_data(regs, cmd, 2).map(|data| data & 0x00ff_ffff)
}
...
pub(super) fn ram_read_status(regs: Regs, cmd: u8) -> Result<u8, ()> {
    ...
    // 1 byte.
    ram_read_data(regs, cmd, 0).map(|data| (data & 0xff) as u8)
}
...
pub(super) fn ram_program_chunk(
    ...
    if !ram_issue_simple_cmd(regs, wren_cmd) {
        return false;
    }
    ...
    ram_run_cmd2_status_poll(regs, read_status_cmd, addr, program_cmd)
}
...
pub(super) fn ram_erase_sector(
    ...
    ram_run_cmd2_status_poll(regs, read_status_cmd, addr, erase_cmd)
}
...
pub(super) fn ram_erase_chip(
    ...
    ram_configure_simple_cmd(regs);
    ram_run_cmd2_status_poll(regs, read_status_cmd, 0, chip_erase_cmd)
}
...
define_ram_irq_wrappers! {
    fn ram_wrapper_read_command_stream(
        ...
        dummy_cycles: u8,
        out: &mut [u8],
    ) -> bool = ram_read_command_stream(regs, cmd, addr, addr_size, dummy_cycles, out);
    fn ram_wrapper_read_jedec_id(regs: Regs, cmd: u8) -> Result<u32, ()>
        = ram_read_jedec_id(regs, cmd);
    fn ram_wrapper_read_status(regs: Regs, cmd: u8) -> Result<u8, ()>
        = ram_read_status(regs, cmd);
    #[allow(clippy::too_many_arguments)]
    ...
    fn ram_wrapper_erase_chip(
        ...
        chip_erase_cmd: u8,
        read_status_cmd: u8,
    ) -> bool = ram_erase_chip(regs, wren_cmd, chip_erase_cmd, read_status_cmd);
    fn ram_wrapper_issue_simple_cmd(regs: Regs, cmd: u8) -> bool
        = ram_issue_simple_cmd(regs, cmd);
    fn ram_wrapper_wait_ready(regs: Regs, read_status_cmd: u8) -> bool
        = ram_wait_ready_sme1(regs, read_status_cmd);
}
